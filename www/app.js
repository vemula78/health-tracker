...
const healthkitBtn = document.getElementById("healthkit-sync");

// Show HealthKit button only in native Capacitor app
if (typeof window.Capacitor !== "undefined" && window.HealthKit) {
  healthkitBtn.style.display = "inline-block";

  healthkitBtn.addEventListener("click", async () => {
    const btn = healthkitBtn;
    const originalText = btn.textContent;
    btn.textContent = "Syncing...";
    btn.disabled = true;

    try {
      const data = await window.HealthKit.getTodayData();
      if (data && Object.keys(data).some(k => data[k])) {
        importHealthText(JSON.stringify(data));
...
// Listen for automatic sync events
window.addEventListener("healthKitDataAvailable", (event) => {
  importHealthText(JSON.stringify(event.detail));
});
